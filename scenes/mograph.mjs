...
const cache = new Map();
const nameOf = () => new URLSearchParams(location.search).get('mograph') || 'demo';
...
export function create(THREE, ctx) {
  ...
  // style tunables (GAN-loop knobs): multipliers the adversarial judge can turn
  const STYLE = Object.assign({ glow: 1, shadow: 1, lineBold: 1, textScale: 1 }, d.style || {});
  ...
  const builders = {
    statPop(el, b) {
      ...
      const big = mk(el, `position:absolute;left:50%;top:44%;transform:translate(-50%,-50%);font-weight:900;font-size:${Math.round(Math.min(H * 0.42, W * 0.22) * STYLE.textScale)}px;letter-spacing:-0.03em;white-space:nowrap;`, String(b.text || '0'));
    ...
    bubbles(el, b) {
      ...
      const discEls = words.map((w, i) => {
        ...
        const bloom = S('circle'); bloom.setAttribute('cx', 0); bloom.setAttribute('cy', 0); bloom.setAttribute('r', rDisc * 1.18); bloom.setAttribute('fill', '#fff'); bloom.style.filter = 'blur(10px)'; bloom.setAttribute('opacity', String(0.45 * STYLE.glow));
      ...
      // PHASE 1 layer: lens-reveal glow (Module 04: filled white nodes with glowing text)
      const wordG = S('g'); wordG.setAttribute('clip-path', `url(#${uid}c)`); svg.appendChild(wordG);
      const wordEls = words.map((w, i) => {
        const g = S('g'); g.setAttribute('opacity', '0');
        const rc = S('circle'); rc.setAttribute('cx', wp[i].x); rc.setAttribute('cy', wp[i].y); rc.setAttribute('r', rGlow); rc.setAttribute('fill', '#fff'); rc.setAttribute('stroke', '#fff'); rc.setAttribute('stroke-width', '2');
        const gl = S('circle'); gl.setAttribute('cx', wp[i].x); gl.setAttribute('cy', wp[i].y); gl.setAttribute('r', rGlow * 1.35); gl.setAttribute('fill', `url(#${uid}g)`);
        const tx = S('text'); tx.setAttribute('x', wp[i].x); tx.setAttribute('y', wp[i].y); tx.setAttribute('text-anchor', 'middle'); tx.setAttribute('dominant-baseline', 'central'); tx.setAttribute('fill', '#000'); tx.setAttribute('font-weight', '800'); tx.setAttribute('font-size', Math.round(H * 0.045)); tx.textContent = String(w);
      ...
      return (lt) => {
        ...
        // LENS: Module 04 Z-Space Masking (Sweep from bottom-left to top-right)
        const sweepP = sm(0.0, P1, lt);
        const lxSearch = lerp(W * 0.15, W * 0.85, sweepP);
        const lySearch = lerp(H * 0.85, H * 0.25, sweepP);
        
        const cx = lt < P1 ? lxSearch : lerp(lxSearch, W / 2, e);   // drift toward centre as it grows
        const cy = lt < P1 ? lySearch : lerp(lySearch, H / 2, e);
        ...
        // word reveal glows: reveal based on proximity to sweeping lens (Module 04 Inversion)
        wordEls.forEach((g, i) => {
          const dist = Math.hypot(cx - wp[i].x, cy - wp[i].y);
          const on = (lt < P1 && dist < lensR * 1.1) ? sm(0, 1, 1 - (dist / (lensR * 1.1))) : 0;
    ...
    docGrid(el, b) {
      ...
      for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++) {
        ...
        docs.push({ cell, back, front, i, r, c, tilt: (hash(i) - 0.5) * 14,
      ...
      return (lt) => {
        const ts = lt * beatSec;
        docs.forEach((dd) => {
          // Module 05: Index-based t_delay matrix generation: (r + c) * 0.04
          const delay = (dd.r + dd.c) * 0.04;
          const p = outCubic(clamp01((lt - delay) / 0.24));
    ...
    gridCard(el, b) {
      ...
      const mkLine = (x1, y1, x2, y2, color) => {
        ...
        p.setAttribute('fill', 'none'); p.setAttribute('stroke', color); p.setAttribute('stroke-width', H * 0.026 * STYLE.lineBold); p.setAttribute('stroke-linecap', 'round');
    ...
    particles(el, b) {
      ...
      return (lt) => {
        ...
        halo.style.opacity = Math.min(1, sm(0.1, 0.35, lt) * (1 - sm(0.9, 1, lt)) * (0.8 + 0.2 * Math.sin(lt * 20)) * STYLE.glow);
        if (cap) cap.style.opacity = sm(0.3, 0.5, lt);
      };
    }
  };
  ...
  // Helpers for continuous logo rejection sampling
  ...
  const distArcT = (px, py, cx, cy, R) => py >= cy ? Math.abs(Math.hypot(px - cx, py - cy) - R) : Math.min(Math.hypot(px - (cx - R), py - cy), Math.hypot(px - (cx + R), py - cy));
  const distArcB = (px, py, cx, cy, R) => py <= cy ? Math.abs(Math.hypot(px - cx, py - cy) - R) : Math.min(Math.hypot(px - (cx - R), py - cy), Math.hypot(px - (cx + R), py - cy));
  const distLogo = (px, py) => Math.min(
    distSeg(px, py, -3.0, -0.75, -3.0, 1.25), distArcT(px, py, -2.25, 1.25, 0.75),
    distSeg(px, py, -1.5, 1.25, -1.5, -1.25), distArcB(px, py, -0.75, -1.25, 0.75),
    distSeg(px, py, 0.0, -1.25, 0.0, 2.25),   distArcT(px, py, 0.75, 2.25, 0.75),
    distSeg(px, py, 1.5, 2.25, 1.5, -2.25),   distArcB(px, py, 2.25, -2.25, 0.75),
    distSeg(px, py, 3.0, -2.25, 3.0, -0.25)
  );
  ...
  for (let i = 0; i < NP; i++) {
    ...
    // Target Logo (Continuous Snaking Line via Rejection Sampling)
    let lx, ly;
    while(true) {
       lx = (hash(seedState++) - 0.5) * 9.0;
       ly = (hash(seedState++) - 0.5) * 9.0;
       if (distLogo(lx, ly) < 0.38) break; // Stroke thickness radius
    }
    targetLogo[i * 3] = lx;
    targetLogo[i * 3 + 1] = ly;
    targetLogo[i * 3 + 2] = 0;
  ...
  // ---- ONE-WORLD ACTORS: at each boundary the outgoing hero TRANSFORMS into
  // the incoming hero — the SAME element continues (full-fidelity: real rect/
  // circle/text interpolation), not a silhouette stand-in. This is the
  // reference's soul: nothing transitions, things transform.
  const lerpN = (a2, b3, x) => a2 + (b3 - a2) * x;
  const actorFrom = (prev, next) => {
    const t1 = prev.type, t2 = next.type;
    if (t1 === 'statPop' && t2 === 'gridCard') {
      const cw4 = W / 4, ch4 = H / 4;
      return { from: { x: W / 2, y: H * 0.44, w: W * 0.55, h: H * 0.2, br: 0, bg: [0, 0, 0, 0], bw: 0, text: String(prev.text || ''), fs: H * 0.16, ink: P.ink },
               to: { x: cw4 * 1.5, y: ch4 * 1.5, w: 5, h: ch4 * 0.7, br: 2, bg: [255, 255, 255, 1], bw: 0, fs: 0.1 } };
    }
    if (t1 === 'gridCard' && t2 === 'bubbles') {
      const lensR = H * 0.4;
      return { from: { x: W * 0.62, y: H * 0.5, w: W * 0.15, h: H * 0.19, br: 12, bg: [12, 12, 14, 1], bw: 0 },
               to: { x: W * 0.20, y: H * 0.57, w: lensR * 2, h: lensR * 2, br: lensR, bg: [12, 12, 14, 0], bw: 6 } };
    }
    if (t1 === 'bubbles' && t2 === 'docGrid') {
      const cols2 = Math.min(10, next.cols || 7), rows2 = Math.min(4, next.rows || 3);
      const gx = W / (cols2 + 1), gy = (H * 0.72) / (rows2 + 1), dw = Math.round(W * 0.055);
      return { from: { x: W * 0.8, y: H * 0.57, w: H * 0.29, h: H * 0.29, br: H * 0.145, bg: [255, 255, 255, 1], bw: 0 },
               to: { x: gx, y: H * 0.08 + gy, w: dw, h: Math.round(dw * 1.3), br: 3, bg: [255, 255, 255, 1], bw: 0 } };
    }
    if (t1 === 'docGrid' && t2 === 'cardFlow') {
      return { from: { x: W * 0.44, y: H * 0.44, w: Math.round(W * 0.055), h: Math.round(W * 0.0715), br: 3, bg: [255, 255, 255, 1], bw: 0 },
               to: { x: W * 0.16, y: H * 0.56, w: H * 0.09, h: H * 0.09, br: 14, bg: [255, 255, 255, 1], bw: 0 } };
    }
    if (t1 === 'cardFlow' && t2 === 'particles') {
      // Gemini strip-decode of the reference's signature morph: ring COLLAPSES
      // to a dot (sharp ease-in) -> dot travels -> EXTRUDES into the bar-logo
      // (ease-out). Runs as a dedicated multi-stage actor with a longer bridge.
      return { kind: 'nodeMorph', E: 0.28,
               from: { x: W * 0.82, y: H * 0.56, d: H * 0.52 },
               to: { x: W / 2, y: H * 0.46, d: H * 0.17 } };
    }
    if (t1 === 'particles' && t2 === 'statPop') {
      return { from: { x: W / 2, y: H * 0.46, w: H * 0.17, h: H * 0.17, br: H * 0.085, bg: [255, 255, 255, 1], bw: 0 },
               to: { x: W / 2, y: H * 0.44, w: 6, h: 6, br: 3, bg: [255, 255, 255, 0], bw: 0 } };
    }
    return null;
  };
  const actorSpecs = beats.map((b2, i) => i === 0 ? null : actorFrom(beats[i - 1], b2));
  const actor = mk(o, 'position:absolute;left:0;top:0;display:none;box-sizing:border-box;border:0 solid #fff;border-style:solid;white-space:nowrap;text-align:center;font-weight:900;overflow:hidden;pointer-events:none;');
  // the nodeMorph multi-stage actor: a disc that collapses, travels, and
  // extrudes the 5-bar engine logo (Gemini's strip-decode of the reference)
  const nmG = mk(o, 'position:absolute;left:0;top:0;display:none;pointer-events:none;');
  const nmDisc = mk(nmG, 'position:absolute;border-radius:50%;background:#fff;');
  const NM_BARS = [0.4, 0.72, 0.52, 0.9, 0.6];
  const nmBars = NM_BARS.map(() => mk(nmG, `position:absolute;border-radius:999px;background:${P.accent};opacity:0;`));
  ...
  return {
    update(frame) {
      ...
      const EB = (actorSpecs[k] && actorSpecs[k].E) || E;   // per-boundary bridge width
      const bridging = k > 0 && lt < EB;
      const p = k === 0 ? 1 : outCubic(lt / EB);           // bridge progress
      ...
      if (bridging && actorSpecs[k] && actorSpecs[k].kind === 'nodeMorph') {
        // COLLAPSE (sharp ease-in) -> TRAVEL -> EXTRUDE bars (ease-out)
        const F = actorSpecs[k].from, T = actorSpecs[k].to;
        const lp = clamp01(lt / EB);                       // LINEAR clock: each phase owns its easing
        const pa = clamp01(lp / 0.30), pb = clamp01((lp - 0.30) / 0.25), pc = clamp01((lp - 0.55) / 0.45);
        const ei = (x2) => x2 * x2, eo = (x2) => 1 - Math.pow(1 - x2, 3);
        const dot = F.d * 0.11;
        const dd = pc > 0 ? lerp(dot, T.d, eo(pc)) : lerp(F.d, dot, ei(pa));
        const ax = lerp(F.x, T.x, eo(pb)), ay = lerp(F.y, T.y, eo(pb));
        nmG.style.display = 'block'; actor.style.display = 'none';
        nmDisc.style.left = (ax - dd / 2) + 'px'; nmDisc.style.top = (ay - dd / 2) + 'px';
        nmDisc.style.width = dd + 'px'; nmDisc.style.height = dd + 'px';
        const barsIn = eo(clamp01((pc - 0.3) / 0.7));
        const bw2 = dd * 0.09, gap = dd * 0.14;
        nmBars.forEach((bar, i2) => {
          const bh = dd * 0.5 * NM_BARS[i2] * barsIn;
          bar.style.opacity = barsIn > 0.02 ? 1 : 0;
          bar.style.left = (ax + (i2 - 2) * gap - bw2 / 2) + 'px';
          bar.style.top = (ay - bh / 2) + 'px';
          bar.style.width = bw2 + 'px'; bar.style.height = bh + 'px';
        });
        nmG.style.opacity = 1 - sm(0.92, 1, lp);
        tpath.style.opacity = 0; tdot.style.opacity = 0; shifter.setAttribute('opacity', 0);
      } else if (bridging && actorSpecs[k]) {
        // ONE-WORLD: drive the transformation actor; no abstract ink needed
        const F = actorSpecs[k].from, T = actorSpecs[k].to;
        const ax = lerpN(F.x, T.x, p), ay = lerpN(F.y, T.y, p);
        const aw = lerpN(F.w, T.w, p), ah = lerpN(F.h, T.h, p);
        actor.style.display = 'block'; nmG.style.display = 'none';
        actor.style.left = (ax - aw / 2) + 'px'; actor.style.top = (ay - ah / 2) + 'px';
        actor.style.width = aw + 'px'; actor.style.height = ah + 'px';
        actor.style.borderRadius = lerpN(F.br, T.br, p) + 'px';
        const bgc = [0, 1, 2, 3].map((i2) => lerpN(F.bg[i2], T.bg[i2], p));
        actor.style.background = 'rgba(' + (bgc[0] | 0) + ',' + (bgc[1] | 0) + ',' + (bgc[2] | 0) + ',' + bgc[3].toFixed(3) + ')';
        actor.style.borderWidth = lerpN(F.bw || 0, T.bw || 0, p) + 'px';
        actor.style.opacity = 1 - sm(0.84, 1, p);
        if (F.text) {
          actor.textContent = F.text; actor.style.color = F.ink || '#fff';
          actor.style.fontSize = Math.max(1, lerpN(F.fs, T.fs || 1, p)) + 'px';
          actor.style.lineHeight = ah + 'px';
        } else { actor.textContent = ''; }
        tpath.style.opacity = 0; tdot.style.opacity = 0; shifter.setAttribute('opacity', 0);
      } else if (bridging) {
        actor.style.display = 'none'; nmG.style.display = 'none';
        const a = focals[k - 1], b2 = focals[k];
      ...
      } else { tpath.style.opacity = 0; tdot.style.opacity = 0; shifter.setAttribute('opacity', 0); actor.style.display = 'none'; nmG.style.display = 'none'; }
